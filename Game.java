...
public class Game
{
     ...
     * @throws GameException 如果铸币不足3或者随从已满7个，抛出GameException
     */
    public void enroll(int playerID, Fellow f, int position) throws GameException
    {
        ...
        if(store.getCoin()<3) throw new GameException("铸币不足，无法购买。");
        if(battlefield.size()>=7) throw new GameException("随从数量已满7个，无法购买。");
        store.enroll(f);
    ...
    public void sell(int playerID, Fellow f) throws GameException
    {
        Store store = storesList.get(playerID);
        ArrayList<Fellow> battlefield = battlefieldsList.get(playerID);
        if(battlefield.contains(f)==false) throw new GameException("您在试图卖出一个自己没有的随从！");
        store.sell(f);
    ...
    {
        Store store = storesList.get(playerID);
        if(store.getCoin()<1) throw new GameException("铸币不足，无法刷新商店。");
        store.refresh();
    }
    ...
    {
        Store store = storesList.get(playerID);
        if(store.getCoin()<store.getUpgradeFee()) throw new GameException("铸币不足，无法升级商店。");
        store.upgrade();
    }
    ...
    public void cast(int playerID, Fellow f, int position) throws GameException
    {
        ...
        ArrayList<Fellow> battlefield = battlefieldsList.get(playerID);
        if(inventory.contains(f)==false) throw new GameException("您在试图施放一个您没有的随从！");
        inventory.remove(f);
        battlefield.add(position, f);
    }
